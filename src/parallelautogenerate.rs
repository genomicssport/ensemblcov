...
pub fn threadedautogenerate(generate: &str) -> Result<String, Box<dyn Error>> {
    ...
    if generate == "yes" {
        ...
        let _unzipcommand = Command::new("gunzip")
            ...
            .expect("command failed");
        thread::spawn(move || {
            let mut collectvar = linecollect("gencode.v48.chr_patch_hapl_scaff.annotation.gtf");
            collectfinaliter.clone().push(collectvar);
        });
    };

    let mut filewrite = File::create("ensembl-id.txt").expect("the file not present");
    writeln!(filewrite, "{}\t{}", "ensemblid", "geneid");
    for i in collectfinaliter.iter() {
        for val in i.iter() {
            writeln!(filewrite, "{}\t{}", val.0, val.1);
        }
    }
